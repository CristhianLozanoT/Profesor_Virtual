...
def responder_con_gemini(pregunta, tema, historial="",es_inicio=False, nombre_estudiante="Estudiante"):
    """
   Genera respuestas usando Gemini con saludo inicial y resumen del tema
      Args:
        pregunta (str): Pregunta del usuario
        tema: Objeto Tema con la información del documento
        historial (str): Historial de la conversación
        es_inicio (bool): Indica si es el primer mensaje de la conversación
        nombre_estudiante (str): Nombre del estudiante para personalización
  """
    contexto = recuperar_fragmentos_relevantes(pregunta, tema)
    
      if es_inicio:
        # Corregimos la sintaxis del prompt_inicial eliminando el = extra
        prompt_inicial = f"""Eres un profesor virtual amigable. Usando el siguiente contexto, genera:
        1. Un saludo personal al estudiante {nombre_estudiante}
        2. Una breve introducción (2-3 oraciones) sobre el tema general del documento
        3. Un resumen conciso de los principales puntos que se tratarán

        === CONTEXTO DEL DOCUMENTO ===
        {contexto}

        Responde de manera amigable y motivadora, invitando al estudiante a hacer preguntas."""

        try:
            respuesta_inicial = modelo_llm.generate_content(prompt_inicial)
            if respuesta_inicial and respuesta_inicial.text:
                return respuesta_inicial.text, contexto
            else:
                return "Lo siento, no pude generar una respuesta inicial.", contexto
        except Exception as e:
            print(f"Error en respuesta inicial: {e}")
            return "Lo siento, ocurrió un error al generar el saludo inicial.", contexto
