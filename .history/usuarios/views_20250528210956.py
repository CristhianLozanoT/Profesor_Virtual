...
class PreguntaView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        try:
            ...
            pregunta = serializer.validated_data.get('pregunta', '')  # Make pregunta optional
            ...
            # Get conversation history if not initial message
            historial_texto = ""
            if not es_inicio:
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            # Get conversation ID from instance
            instance = self.get_serializer().instance
            if instance and instance.id_conversacion:
                id_conversacion = instance.id_conversacion
            else:
                id_conversacion = str(uuid.uuid4())
            ...
            return Response(data, status=status.HTTP_201_CREATED)  # Use status from rest_framework

        except Exception as e:
            print(f"Error in create: {str(e)}")
            return Response(
                {'error': 'Error al procesar la solicitud'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR  # Use status from rest_framework
            )
